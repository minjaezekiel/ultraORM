...
User.fields = {
  ...
  countryid: new IntegerField({ nullable: true }),
...
// 3. Post Model (One-to-Many with User)
...
// 5. UserRole Model (Junction table for User-Role Many-to-Many)
...
// 7. Category Model (Self-referential)
...
// 9. Comment Model (Polymorphic - can belong to Post OR Video)
...
// 11. Taggable Model (Polymorphic junction table)
...
async function testAllRelations() {
  ...
  try {
    ...
    const assignments = [
      { '"userId"': createdUsers[0].get('id'), '"roleId"': createdRoles[0].get('id') }, // John is Admin
      { '"userId"': createdUsers[0].get('id'), '"roleId"': createdRoles[1].get('id') }, // John is also Editor
      { '"userId"': createdUsers[1].get('id'), '"roleId"': createdRoles[1].get('id') }, // Jane is Editor
      { '"userId"': createdUsers[1].get('id'), '"roleId"': createdRoles[2].get('id') }, // Jane is also Viewer
      { '"userId"': createdUsers[2].get('id'), '"roleId"': createdRoles[2].get('id') }, // Bob is Viewer
      { '"userId"': createdUsers[3].get('id'), '"roleId"': createdRoles[2].get('id') }  // Alice is Viewer
    ];
    ...
    for (const t of taggables) {
      ...
      await taggable.save();
      console.log(`   ✅ Added #${createdTags.find(tg => tg.get('id') === t['"tagId"']).get('name')} to ${t.taggable_type}`);
    }
    ...
    createdCategories[1].set('parentId', createdCategories[0].get('id')); // Computers under Electronics
    createdCategories[2].set('parentId', createdCategories[1].get('id')); // Laptops under Computers
    createdCategories[3].set('parentId', createdCategories[1].get('id')); // Desktops under Computers
    ...
    console.log(`   Author: ${postWithComments.author.get('name')}`);
    ...
    console.log('\n11. COMPLEX NESTED INCLUDES: User with Profile, Posts, and Comments');
    const complexUser = await User.query()
      .include(['profile', 'posts', 'roles'])
      .where({ id: createdUsers[0].get('id') })
      .first();
    console.log(`   User: ${complexUser.get('name')}`);
    console.log(`   Profile: ${complexUser.profile ? complexUser.profile.get('bio') : 'None'}`);
    console.log(`   Roles: ${complexUser.roles.map(r => r.get('name')).join(', ')}`);
